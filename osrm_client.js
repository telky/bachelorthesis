...
module.exports = {
    ...
    routeToB: function (fromCoordinates, toCoordinates, callback) {
        ...
        var accessId = ';accessId=bosch-Thiele-4035-900f-29403c078264';
        var poly = 'poly=1;'

        // sets the departure date for the search (default: server date)
        var date = 'date=2019-08-08;';

        // sets the departure time for the search (default: server time)
        var time = 'time=10:00:00;';


        request(baseUrl + poly + date + time + fromLat + fromLatCoords + ';' + fromLong + fromLongCoords + ';' + destLat + destLatCoords + ';' + destLong + destLongCoords + accessId, { json : true}, (err, res, body) => {

            if(err) {
                return console.log('VBB ERROR:',err);
            }

            console.log('VBB URL:', baseUrl + poly + date + time + fromLat + fromLatCoords + ';' + fromLong + fromLongCoords + ';' + destLat + destLatCoords + ';' + destLong + destLongCoords + accessId);
            callback(body);
